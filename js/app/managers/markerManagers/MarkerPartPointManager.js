(function(global) {
    ...
    MyApp.MarkerPartPointManager = class MarkerPartPointManager {
        ...
        _createCoreManager(targetTimeRangeType, targetUrl) {
            var layerName = '';
            var callbacks = {
                'tooltipContentCallback': MarkerPartPointManager._buildTooltipContent,
                'popupContentCallback': null,
                'filterGeojsonPredicate': MarkerPartPointManager._filterGeojsonPredicate,
            };
            var options = {
                shouldCircleMarker: true,
                circleMarkerColNames: {radius: 'partPointCircleMarkerRadius', color: 'partPointCircleMarkerColor', isFill: 'partPointCircleMarkerIsFill'},
                shouldTooltip: true,
                tooltipNames: {className: 'tooltip-part-point', geZoom: 'partPointTooltipGeZoom', direction: 'partPointTooltipDirection'},
                shouldPopup: false,
                popupNames: null,
            };
        ...
        static _buildTooltipContent(properties) {
            var content = `
<div>
    <div class="tooltip-part-point-name">${properties.partPointName}</div>
</div>
`;
            return content;
        }
...
`;
        ...
        static _filterGeojsonPredicate(properties) {
            if (properties.isPartPoint === true) {
                return true;
            }
            return false;
        }
